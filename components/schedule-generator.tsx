...
export function ScheduleGenerator() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [employeesRes, scheduleRes, weekDaysRes, shiftRulesRes, shiftsRes, summaryRes] = await Promise.all([
          fetch('https://mocki.io/v1/5554b169-a161-4f99-ab1f-11c4b69b1a94'), // Employees data
          fetch('/schedule.json'),
        ...
        const [employeesData, scheduleData, weekDaysData, shiftRulesData, shiftsData, summaryData] = await Promise.all([
          employeesRes.json(),
          scheduleRes.json(),
          weekDaysRes.json(),
          shiftRulesRes.json(),
  ...
  const totalShifts = summaryData.reduce((total, shift) => total + shift.count, 0)
  ...
  const renderShiftCards = () => {
    ...
    const firstRowShifts = shifts.slice(0, 3);
    const secondRowShifts = shifts.slice(3, 6);
      ...
      <>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 text-center mb-2">
          {firstRowShifts.map((shift, index) => (
            <div 
              key={index} 
              className={`${getShiftColor(shift.label)} rounded-lg p-3 flex flex-col items-center justify-center`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="mb-4 space-y-2">
              {shiftRules.map((rule, index) => (
                <div key={index} className="flex items-center">
                  <div className={`w-3 h-3 rounded-full mr-2 ${index === 0 ? 'bg-blue-200'
          ...
          <div className="border rounded-lg p-4">
            <div className="flex items-center justify-between mb-3 p-1">
              ...
              <div className="text-xs text-gray-500">Total shifts: {totalShifts}</div>
            </div>
            {renderSummaryCards()}
          </div>
        </div>
